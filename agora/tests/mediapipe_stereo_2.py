    ...
    while cap.isOpened():
        ...
        for hand_index, hand_pose in mp_hand_pose_dict_left.items():
            for point in hand_pose:
                x, y, z = point
                cv2.circle(frame_left, (int(x), int(y)), 5, handedness_color_map[hand_index], -1)
        
        for hand_index, hand_pose in mp_hand_pose_dict_right.items():
            for point in hand_pose:
                x, y, z = point
                cv2.circle(frame_right, (int(x), int(y)), 5, handedness_color_map[hand_index], -1)
